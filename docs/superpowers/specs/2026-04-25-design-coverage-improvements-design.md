...
The single biggest design improvement is a cross-cutting one: **sealed-enum-driven platform-hint frontmatter, generated per repo by the scout**. Today, "what counts as a permission hotspot" is agent-judgment per stage; tomorrow, it is hint-file data the scout populates per repo. The shipped iOS and Android hints (which were extracted from MindBodyPOS and Express-Android respectively) are removed — they were tuned for two specific apps and silently degraded on any other repo. The scout becomes the single source of platform knowledge; every consuming repo gets its own scout-generated, scout-reviewed, repo-committed hint file.
...
- **Make adding a new platform mechanical, not magical.** Adding a "Platform sections" entry to the scout's stage MD is enough; the design-coverage skill itself doesn't ship per-platform files. The scout uses that platform-truth to generate a repo-specific hint at first run.
- **Make every consuming repo own its hint.** The hint file lives in the consuming repo at `<repo>/.claude/skills/design-coverage/platforms/<auto-name>.md`, generated by the scout on first run, reviewed and committed by the user. The skill itself ships zero per-repo platform hints.
...
The skill defines several sealed enums in its schemas. Four of them (`hotspot.type`, `inventory_item.kind`, `inventory_item.source.surface`, `unwalked_destinations.reason`) require **platform-specific code patterns** to be useful. Today those patterns live as prose under stage sections in two shipped files (`platforms/ios.md`, `platforms/android.md`). Two problems with that:

1. The shipped files are tuned for two specific apps, not for "iOS" or "Android" generally — running on a different iOS or Android repo silently degrades.
2. The patterns are prose, so stage 02 has to ask the agent to recall idioms instead of iterating mechanically.

The redesign fixes both: promote the patterns to structured frontmatter, AND make the hint file repo-local (generated by the scout on first run, committed to the consuming repo). The shipped per-platform files go away.

The structured frontmatter enables:

1. **Stage 02** can mechanically iterate enum values and grep for each repo's patterns instead of asking the agent to recall idioms.
2. **Stage 03** can deterministically emit one Q per `hotspot.type` value that has any matches in stage 02.
3. **The scout** can detect each enum value's patterns when generating a hint for a new repo.

**Frontmatter shape (this is what the scout writes to `<repo>/.claude/skills/design-coverage/platforms/<auto-name>.md`; the example below shows what the scout would emit for a repo it identified as MindBodyPOS-style iOS):**
...
## 01 Flow locator
(scout-generated prose explaining what was found in this specific repo, for human review)
```

The prose body in the generated hint file is for human review — humans read it to understand what the scout inferred about their specific repo. The structured frontmatter is the **machine-actionable index** the analysis stages consume.

The scout's `stages/02-pattern-extraction.md` walks each enum value returned by `get_sealed_enum_pattern_keys()` (a new function in `lib/sealed_enum_index.py`) and produces candidate `grep:` lists by combining (a) platform-truth heuristics encoded in the scout's stage MD with (b) actual symbol scanning of the consuming repo. The result is a hint that knows about both the platform's general conventions AND the specific repo's variations.
...
#### 10b — Remove shipped per-platform hint files; relocate to historical notes

The shipped `platforms/ios.md` (extracted from MindBodyPOS PR #5349) and `platforms/android.md` (extracted from Express-Android PR #5190) are removed from the skill's runtime path. They were a category error: their names implied "this is the iOS hint" / "this is the Android hint" when they were actually hints tuned for two specific apps that happened to be on those platforms. Running design-coverage on a different iOS or Android repo would silently degrade because the patterns wouldn't match.

Action:
- `git mv skills/design-tooling/design-coverage/platforms/ios.md docs/design-coverage/historical-platform-notes/mindbodypos-ios.md`
- `git mv skills/design-tooling/design-coverage/platforms/android.md docs/design-coverage/historical-platform-notes/express-android.md`
- The `platforms/` directory ships empty (or with a README explaining that hints are repo-local, generated by the scout).
- The relocated files exist as **human reference material only** — never read by the skill or the scout. They document what platform knowledge looked like before the scout absorbed it.
- The renaming makes provenance honest. A future contributor reading `docs/design-coverage/historical-platform-notes/mindbodypos-ios.md` immediately knows "this was MindBodyPOS-specific, not universal iOS knowledge."

#### 10c — Move platform knowledge into the scout; generate repo-local hints

The scout becomes the single source of truth for platform knowledge. The `design-coverage-scout/stages/02-pattern-extraction.md` stage gets new responsibilities:

**Platform sections in scout stage prose.** The stage MD gets one section per detected platform (iOS, Android, Flutter, React Native, web — extensible). Each section enumerates the platform-truth that transcends any specific app:

- iOS section: "feature flags cluster around `*FeatureFlag*` / `*Manager*` / `*Switch*` naming; permissions cluster around `staff.can*` / `user.hasRole(` / Apple framework auth APIs (`AVCaptureDevice`, `CLLocationManager`, `UNUserNotificationCenter`, `PHPhotoLibrary`); navigation uses `pushViewController` / `present` / storyboard segues / SwiftUI `NavigationLink`; hybrid hosts use `UIHostingController(rootView:)` and `UIViewControllerRepresentable`."
- Android section: equivalent for Compose, View XML, ViewBinding, navigation graphs, Hilt-injected feature flags, etc.
- New platforms: a contributor adds a new section to scout's stage MD; no shipped template to update.

**Pattern detection algorithm.** For each key returned by `get_sealed_enum_pattern_keys()`:
1. Detect the platform from the unfamiliar codebase (existing detect: glob logic).
2. Look up the relevant Platform section in scout stage prose for inductive bias on what to grep for.
3. Run discovery against the codebase: combine platform-truth heuristics from step 2 with this-codebase-specific symbol scanning (e.g., grep for class definitions matching the heuristic, then promote the actual class names found into the hint).
4. Emit candidate `grep:` patterns specific to the codebase being analyzed.
5. Persist to the draft hint's frontmatter under `sealed_enum_patterns.{key}`.

**The hint file is repo-local.** The draft + approved hint lands at:

```
<repo>/.claude/skills/design-coverage/platforms/<auto-name>.md
```

`<auto-name>` is the repo's slug (e.g., `mindbodypos-ios`, `express-android`, `acme-banking-flutter`). The file is committed by the user and becomes part of their repo's permanent design-coverage configuration. Subsequent design-coverage runs in the same repo find this hint and skip the scout step.

**SKILL.md unknown-stack branch — rewrite (sub-deliverable of #10c).** The current three-option menu (generate via scout / name from list / agnostic) collapses to a single auto-action. New behavior:

```
no repo-local hint at <repo>/.claude/skills/design-coverage/platforms/ →
  invoke design-coverage-scout (live, in-session) →
  scout draft → user reviews/approves draft →
  scout writes <repo>/.claude/skills/design-coverage/platforms/<auto-name>.md →
  design-coverage continues with the new hint
```

If scout fails or the user rejects the entire draft: **abort the run**. No silent agnostic fallback. The `--platform agnostic` flag is removed from arg parsing. (The `--platform <name>` flag is retained for the case where a user wants to override which repo-local hint to use, e.g., a monorepo with multiple subprojects each having their own hint.)
...
- The relevant schemas (`inventory_item.json`, `code_inventory.json`) carry `x-platform-pattern` annotations on every field whose enum values need platform-specific grep patterns.
- The platform-hint frontmatter validator accepts the new fields and rejects `sealed_enum_patterns` keys not returned by the derivation function.
- `skills/design-tooling/design-coverage/platforms/` no longer ships any populated `.md` files (a `README.md` explaining the repo-local pattern is allowed).
- `docs/design-coverage/historical-platform-notes/` contains the renamed files (`mindbodypos-ios.md`, `express-android.md`).
- `design-coverage-scout/stages/02-pattern-extraction.md` contains a "Platform sections" sub-document covering at least iOS and Android, with hooks for adding new platforms.
- The scout produces a draft hint at `<repo>/.claude/skills/design-coverage/platforms/<auto-name>.md` for any platform it has knowledge of, with `sealed_enum_patterns` populated for at least 80% of derived keys.
- SKILL.md's unknown-stack branch performs the auto-invocation flow above; the agnostic option is removed; the `--platform agnostic` flag is removed.
- A smoke-test run against a fresh repo (no existing hint) successfully generates a hint via scout, then completes the design-coverage analysis using that hint.
...
- Wave 2: `[design-coverage] Wave 2 — Repo-local hints generated by scout, sealed-enum frontmatter, schema validation, scratch policy (#7, #10, #11)`
...
- **Scout becomes a hard dependency for every first-run.** No fallback to a shipped hint or to agnostic. If scout fails on an unfamiliar codebase, the run aborts. Mitigation: scout's stage MD encodes platform-truth heuristics for major platforms (iOS, Android, Flutter, RN, web), making first-run scout passes high-fidelity for those platforms. For genuinely novel platforms, the user must contribute a "Platform sections" entry to the scout stage MD before the skill works on that platform — this is intentional friction; better than producing a confidence-laden but wrong report. The existing scout draft/approve gate ensures the user sees what was inferred before any analysis runs.
- **First-run friction.** Every new repo's first design-coverage run includes a scout pass before analysis. Estimated added time: 5-15 minutes plus the user's review of the scout draft. Mitigation: the hint file is committed to the repo, so subsequent runs in the same repo skip scout entirely. Only the first run pays the cost.
- **Scout quality on a fresh repo dictates analysis quality.** A bad scout pass produces a bad hint, which produces a bad analysis. Mitigation: the draft/approve gate forces user review; the hint file is git-tracked, so updates are visible in code review; the schema-derived registry ensures scout knows which enum values it must detect (no silent enum-value drift).
