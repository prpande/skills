...
- **Dogfood prerequisites.** Running the full skill end-to-end against an existing Figma → code modernization requires **a local checkout of the target repo** (stages 2 and 3 walk the code). On-demand file reads via `gh api` would work in principle but are slow (one round-trip per ripgrep query, many round-trips per stage 2) and are NOT exercised on day one. The `docs/design-coverage-dogfood/2026-04-22-appointment-details-smoke/comparison.md` artifact documents a partial smoke against MindBodyPOS #5349's Figma URL run from our worktree — Figma inventory executed via MCP, stages 2/3 evaluated architecturally only.
