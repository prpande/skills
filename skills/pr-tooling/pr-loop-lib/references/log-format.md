...
| `wait_skipped` | `{iteration, reason}` — emitted by step 01 Mode S when the first-iteration wait is bypassed via the `no_wait_first_iteration` flag. `reason` is `"no_wait_first_iteration"`. |
| `webhook_event_received` | `{iteration, event_type}` — emitted by step 01 Mode E when a `<github-webhook-activity>` message is detected in the current turn. `event_type` is the GitHub event name (e.g., `pull_request_review_comment`, `check_run`) or `"unknown"` if not discernible. GitHub only. |
| `webhook_subscribed` | `{iteration, pr_number}` — emitted by step 01 Mode W (GitHub path) when `mcp__github__subscribe_pr_activity` succeeds. |
| `webhook_subscribe_failed` | `{iteration, pr_number}` — emitted by step 01 Mode W (GitHub path) when `mcp__github__subscribe_pr_activity` fails (network error, permission error, tool unavailable). The loop continues with the fallback `ScheduleWakeup` as the primary wait mechanism for this iteration; `context.webhook_subscribed` remains `false`. |
| `wait_clamped` | `{requested_minutes, effective_minutes, reason}` — emitted by `pr-loop-lib/steps/01-wait-cycle.md` when `context.wait_override_minutes` is below the platform floor and gets clamped up. On GitHub: floor is 1 minute, `effective_minutes` is `1`. On AzDO: floor is 10 minutes, `effective_minutes` is `10`. `reason` is a short string identifying the floor (e.g., `"minimum fallback interval (webhook-primary path)"` or `"reviewer-bot response window (AzDO polling path)"`). Informational only — the loop continues with the clamped value. |
| `wait_cycle` | `{iteration, mode, fallback_seconds}` (GitHub) or `{iteration, mode, delay_seconds}` (AzDO) — emitted by step 01 Mode W before yielding. `mode` is `"webhook"` on GitHub or `"azdo-poll"` on AzDO. |
| `wait_fallback_fired` | `{iteration, delay_seconds}` — emitted by step 01 wakeup re-entry detection when `ScheduleWakeup` fires (no `<github-webhook-activity>` event arrived within the fallback/polling window). `delay_seconds` is re-derived from `context.wait_override_minutes` using the same formula as Mode W step 2. |
| `webhook_unsubscribed` | `{pr_number}` — emitted by step 11 after `mcp__github__unsubscribe_pr_activity` is called. GitHub only (skipped on AzDO). |
