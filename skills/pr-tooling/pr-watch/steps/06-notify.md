...
| CI needs you | `Needs you: [<check name>](<link>) is red at <head7> (<reason>).` — put `<check name>` and `<link>` through step 2's `<` replacement of "Quoting someone else's text" first, and `<reason>` through steps 2 and 3 whatever it carries: a stderr line and a fixer's own reason are both free text from outside this session. Then, only when the PR's `ci_log_occurrence` is this check's occurrence key, up to 20 lines from `grep -m 20 -E -e '##\[error\]' -e 'error [A-Z]+[0-9]+' -e 'Failed ' -e 'Test Run Failed'` over that redacted file, put through steps 2 and 3 of "Quoting someone else's text" (the file is already redacted), in a code block; the session never reads the whole file |
