...
| CI needs you | `Needs you: [<check name>](<link>) is red at <head7> (<reason>).` — put `<check name>` and `<link>` through step 2's `<` replacement of "Quoting someone else's text" first, and `<reason>` through steps 2 and 3 whatever it carries: a stderr line and a fixer's own reason are both free text from outside this session. Then, only when `<reason>` is neither `no log access` nor starts with `no log access: ` (07 rule 2 and rule 3's escalations that never wrote a fresh log for this occurrence — the file, if it exists, is a leftover from an earlier check) and `<worktree>/.pr-autopilot/ci-log-<N>.txt` exists, up to 20 lines from `grep -m 20 -E -e '##\[error\]' -e 'error [A-Z]+[0-9]+' -e 'Failed ' -e 'Test Run Failed'` over that redacted file, put through steps 2 and 3 of "Quoting someone else's text" (the file is already redacted), in a code block; the session never reads the whole file |
